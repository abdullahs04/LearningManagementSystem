...
# Script to check specific admin files for TypeScript errors
# Ignoring specific configuration warnings

echo "Checking admin-specific files for errors..."

cd frontend || { echo "Error: frontend directory not found"; exit 1; }

FILES_TO_CHECK=(
  "src/pages/admin-dashboard/AttendanceRecords.tsx"
  "src/pages/admin-dashboard/classAndSubmanagement.tsx"
  "src/pages/admin-dashboard/FeeManagement.tsx"
  "src/pages/admin-dashboard/gradebook.tsx"
  "src/pages/admin-dashboard/Overview.tsx"
  "src/pages/admin-dashboard/staffManagement.tsx"
  "src/pages/admin-dashboard/SystemSettings.tsx"
  "src/pages/admin-dashboard/TimetableAndExams.tsx"
)
...
for file in "${FILES_TO_CHECK[@]}"; do
  if [ -f "$file" ]; then
    ...
    ERROR_OUTPUT=$(npx tsc --noEmit "$file" 2>&1 | \
      ...
      grep -v "JSX element class does not support attributes because it does not have a 'props' property" | \
      grep -v "cannot be used as a JSX component" | \
...
if [ $ERROR_FOUND -eq 0 ]; then
  echo -e "\n\033[0;32m✅ All admin files passed validation!\033[0m"
  exit 0
else
