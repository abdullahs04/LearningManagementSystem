...
export default function AttendanceRecords() {
  ...
  const courses = [
    { id: 1, code: "CS101", name: "Introduction to Computer Science", totalClasses: 16, attendedClasses: 14 },
    { id: 2, code: "MATH201", name: "Calculus II", totalClasses: 18, attendedClasses: 15 },
    { id: 3, code: "PHY102", name: "Physics for Engineers", totalClasses: 14, attendedClasses: 12 },
    { id: 4, code: "ENG105", name: "Technical Writing", totalClasses: 12, attendedClasses: 10 },
    { id: 5, code: "CHEM101", name: "General Chemistry", totalClasses: 15, attendedClasses: 11 },
  ];
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-gray-900 lg:p-6">
          <div className="space-y-4">
            ...
            <p className="text-gray-600 dark:text-gray-400">
              View your attendance records for all enrolled courses. Click on a course to see your detailed attendance history.
            </p>
            
            {/* Summary Cards */}
            <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div className="rounded-xl bg-blue-50 p-4 dark:bg-blue-900/30">
                <h3 className="text-sm font-medium text-blue-800 dark:text-blue-300">Total Courses</h3>
                <p className="mt-2 text-2xl font-semibold text-blue-900 dark:text-blue-100">{courses.length}</p>
              </div>
              <div className="rounded-xl bg-blue-50 p-4 dark:bg-blue-900/30">
                <h3 className="text-sm font-medium text-blue-800 dark:text-blue-300">Average Attendance</h3>
                <p className="mt-2 text-2xl font-semibold text-blue-900 dark:text-blue-100">
                  {Math.round(courses.reduce((acc, course) => acc + getAttendancePercentage(course.attendedClasses, course.totalClasses), 0) / courses.length)}%
                </p>
              </div>
            </div>
          </div>
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-2 flex items-center gap-4">
                        <span className="text-sm text-gray-500 dark:text-gray-400">
                          Classes: {course.attendedClasses}/{course.totalClasses}
                        </span>
                        <div className="flex items-center gap-1.5">
                          <div className="h-2 w-24 rounded-full bg-gray-200 dark:bg-gray-700">
                            <div 
                              className="h-2 rounded-full bg-blue-600 dark:bg-blue-500" 
                              style={{ width: `${getAttendancePercentage(course.attendedClasses, course.totalClasses)}%` }}
                            ></div>
                          </div>
                          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">
                            {getAttendancePercentage(course.attendedClasses, course.totalClasses)}%
                          </span>
                        </div>
                      </div>
                    </div>
      ...
      <Modal isOpen={isOpen} onClose={closeModal} size="xl">
        ...
        <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row sm:items-center justify-between rounded-lg bg-blue-50 p-4 dark:bg-blue-900/30">
          <div className="mb-3 sm:mb-0">
            <p className="text-xs sm:text-sm font-medium text-blue-800 dark:text-blue-300">Attendance Rate</p>
            <p className="text-xl sm:text-2xl font-bold text-blue-900 dark:text-blue-100">
            {getAttendancePercentage(selectedCourse.attendedClasses, selectedCourse.totalClasses)}%
            </p>
          </div>
          <div className="flex gap-4 sm:gap-6">
            <div className="text-center">
            <p className="text-xs sm:text-sm font-medium text-green-600 dark:text-green-400">Present</p>
            <p className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">
              {classAttendance.filter(item => item.status === "present").length}
            </p>
            </div>
            <div className="text-center">
            <p className="text-xs sm:text-sm font-medium text-red-600 dark:text-red-400">Absent</p>
            <p className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">
              {classAttendance.filter(item => item.status === "absent").length}
            </p>
            </div>
            <div className="text-center">
            <p className="text-xs sm:text-sm font-medium text-yellow-600 dark:text-yellow-400">Leave</p>
            <p className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">
              {classAttendance.filter(item => item.status === "leave").length}
            </p>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-lg border border-gray-200 dark:border-gray-700">
          <div className="grid grid-cols-12 gap-2 border-b border-gray-200 bg-gray-50 p-3 dark:border-gray-700 dark:bg-gray-800">
            <div className="col-span-2 text-sm font-medium text-gray-700 dark:text-gray-300">Class</div>
            <div className="col-span-4 text-sm font-medium text-gray-700 dark:text-gray-300">Date</div>
            <div className="col-span-6 text-sm font-medium text-gray-700 dark:text-gray-300">Status</div>
          </div>
          <div className="max-h-60 sm:max-h-80 overflow-y-auto">
            ...
            >
              ...
              <div className="col-span-6">
                <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium text-white ${getStatusColor(item.status)}`}>
